...
class ImageFolder:
    ...
    def __init__(self, path, transform=None, img_transform=None, base_transform=None, loader=default_loader):
    ...
    def __getitem__(self, idx: int):
        ...
        ori = img.copy()
        record = {"ori": ori, "input": img, "instance_id": idx}
        if self.img_transform:
            record["input"] = self.img_transform(record["input"])
        if self.transform:
            record = self.transform(record)
        if self.base_transform:
            record["ori"] = self.base_transform(record["ori"])
        return record
